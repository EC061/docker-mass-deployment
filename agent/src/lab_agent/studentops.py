...
from .paths import user_cold, user_scratch
...
def add_student(cfg: AgentConfig, params: dict[str, Any]) -> tuple[Any, str]:
    ...
    # Per-student datasets appear under the container's bind-mounted users parents.
    zfs.create_dataset(user_scratch(cfg, lab, username), quota_bytes=scratch_quota)
    zfs.create_dataset(user_cold(cfg, lab, username), quota_bytes=cold_quota)
...
def remove_student(cfg: AgentConfig, params: dict[str, Any]) -> tuple[Any, str]:
    ...
    if delete_data:
        ...
        zfs.destroy_dataset(user_cold(cfg, lab, username), recursive=True)
