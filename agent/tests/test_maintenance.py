...
def test_run_scrub_skips_smb_cold_storage(monkeypatch):
    ...
    monkeypatch.setattr(maintenance.zfs, "start_scrub", lambda p: started.append(p) or True)
    monkeypatch.setattr(maintenance.zfs, "scrub_status", _ok_status)
    cfg = make_cfg(cold_backend="smb", slow_path="/mnt/cold")
...
def test_node_reboot_is_scheduled_after_result_delivery(monkeypatch):
    ...
    result, _ = maintenance.run_reboot(_cfg(), {})
    assert result == {"scheduled": True, "delay_seconds": 10}
    assert "--on-active=10s" in calls[0]
